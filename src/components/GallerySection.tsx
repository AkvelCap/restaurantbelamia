    ...
    <section id="gallery" className="section-padding bg-card">
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-14">
          <p className="text-warm-gold text-sm font-sans tracking-[0.25em] uppercase mb-3">{t.gallery.tag}</p>
          <h2 className="section-title text-foreground mb-6">{t.gallery.title}</h2>
          <p className="text-muted-foreground font-sans leading-relaxed">{t.gallery.desc}</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
          {images.map((img) => (
            <div key={img.alt} className="group relative overflow-hidden rounded-lg aspect-[4/3]">
              <img src={img.src} alt={img.alt} loading="lazy" width={800} height={600} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
